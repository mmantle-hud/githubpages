...
We can then use this query as a subquery (i.e. inside another query) to give the title the assessment that has the highest weighting. 
...
Again, for the first example, we'll look at the subquery in isolation first. This query retrieves the scores of all the students that passed (i.e. they got a score of 40 or greater) for assessment 2, the 'Programming Assignment'.
...
We can place this query in a `FROM` clause, and select columns to retrieve. By placing the subquery in a `FROM` clause the subquery becomes like a virtual table that we can select columns from or perform aggregations on. 
...
- The key thing here is the subquery will be executed six times. Once for each degree in the database. Each time the subquery is run, a different `degree_id` is passed in. 
...
The following query returns the titles of all the degrees that have at least one student enrolled on the course. 
