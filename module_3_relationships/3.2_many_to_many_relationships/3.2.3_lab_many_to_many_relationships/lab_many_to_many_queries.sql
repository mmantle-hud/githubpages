...
-- Display the title, year, and genres for all movies. The results should have one row per movie (use STRING_AGG() to construct a list of genres)
SELECT m.title,
    m.release_year,
...
GROUP BY m.title,
    m.release_year;
-- Display the title, year, language name and genres for the movie Frozen. Again use STRING_AGG() so your results feature a single row. 
SELECT m.title,
    m.release_year,
    l.language_name,
    STRING_AGG(g.genre_name, ', ') AS genres
FROM movies AS m
    ...
    INNER JOIN genre_movie AS gm ON m.movie_id = gm.movie_id
    INNER JOIN genres AS g ON gm.genre_id = g.genre_id
WHERE m.title = 'Frozen'
GROUP BY m.title,
    m.release_year,
    l.language_name;
-- Display all the Chinese Mandarin films that are categorised as romance
SELECT m.title,
    m.release_year,
    l.language_name,
    g.genre_name
FROM movies AS m
    ...
    INNER JOIN genre_movie AS gm ON m.movie_id = gm.movie_id
    INNER JOIN genres AS g ON gm.genre_id = g.genre_id
WHERE l.language_code = 'cmn'
    AND g.genre_name = 'Romance';
-- Display the number of movies classified under each genre, and the average duration of the movies classified under each genre. Order this list by average duration from longest to shortest.
SELECT g.genre_name,
    COUNT(*) AS num_films,
