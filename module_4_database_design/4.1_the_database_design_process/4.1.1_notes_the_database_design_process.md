...
Again based on the requirements, we can assign attributes to the entities i.e. the properties or characteristics of the entity. 
...
On a conceptual ERD, when we describe entities we use singular i.e. **User** not **Users**. We capitalise the first letter, and we use **CamelCase** for multi-word terms e.g. ChairForRent. Attributes how the same pattern but start with a lowercase letter. 
...
We may have to go through several iterations of the model as we get feedback from stakeholders and clarify the requirements for the database. 
...
We should create a new data dictionary. This captures additional information such as foreign key details and database data types. Importantly, it also records important details that may not be possible to represent on an ERD such as check constraints. For example, only one review is allowed for each user/movie. In the `reviews` table, I can specify the combination of `user_id` and `movie_id` must be unique.  
...
> There isn't universal agreement on the exact distinction between different types of models, especially logical vs physical. For example, some people don't put any data types on the logical model. However, the basic principles are widely agreed on. Just don't be surprised if examples in books and online take slightly different approaches. 
