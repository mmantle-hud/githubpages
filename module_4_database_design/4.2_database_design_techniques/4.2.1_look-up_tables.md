...
In the hotel finder database design exercise we needed to store the category of hotel (boutique, business, budget, luxury). In many ways it makes sense to specify this as an attribute of hotel, in a column named `style`.
...
Whenever an administrator enters the details of a new hotel we are relying on them to enter the style of hotel in exactly the same way it was done previously. Human error will inevitably happen, and we might end up with 'Boutique' being entered as 'Bouteek', 'boutique', 'Bootique' etc.
...
> Although a good design choice in this scenario, we should be wary of needlessly creating separate tables for single attributes. Generally attributes should be implemented as columns in the table that represents the entity.
